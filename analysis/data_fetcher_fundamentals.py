...
def _extract_fundamentals(info, fast_info, price_fallback=None):
    def info_float(keys):
...
def load_fundamentals(symbol_override: str, include_alpha: bool = True) -> dict:
    try:
        ...
        return {}
    return _extract_fundamentals(info, fast_info, price_fallback=price_fallback)


def is_empty_fundamentals(payload: dict) -> bool:
    if not payload:
        return True
    for key in (
        "trailingPE",
...
def fetch_stock_fundamentals(symbol: str, include_alpha: bool = True) -> dict:
    """
